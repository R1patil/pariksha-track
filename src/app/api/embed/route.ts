...
function splitIntoChunks(text: string, chunkSize = 500, overlap = 60): string[] {
  ...
  for (const s of sentences) {
    if ((current + " " + s).length > chunkSize && current.length > 0) {
      chunks.push(current.trim());
      current = current.split(" ").slice(-12).join(" ") + " " + s;
    } else {
      current += (current ? " " : "") + s;
    }
  }
...
export async function POST(req: NextRequest) {
  // ── Admin-only: require internal secret ──────────────────
  ...
  try {
    const { text, classNum, subject, chapterId, chapterTitle, sourceFile, schoolId } = await req.json();
    if (!text || !classNum || !subject || !chapterId) {
    ...
    const cleanText = text.replace(/\s+/g, " ").trim();
    if (cleanText.length < 100) {
      return NextResponse.json({ error: "Text too short. Use a text-based PDF." }, { status: 400 });
    }
    ...
    return NextResponse.json({ success: true, chunksProcessed: inserted, totalChunks: chunks.length });
  } catch (err: any) {
    return NextResponse.json({ error: err.message }, { status: 500 });
  }
}
